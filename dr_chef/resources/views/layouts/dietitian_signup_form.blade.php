...
<head>
    ...
    <link href="{{ asset('css\navbar.css') }}" rel="stylesheet" media="all">
    <link href="{{ asset('css/dietitian_signup.css') }}" rel="stylesheet" media="all">
    <link rel="shortcut icon" type="image/png" href="images/favicon.png">
    <link rel="stylesheet" href="{{ asset('css/footer.css') }}">

    <link rel="icon" type="image/png" href="images/favicon.png">
    <title>Dietitian Signup</title>
</head>
...
<body>
    ...
    <div class="page-wrapper p-b-100" style="padding-top: 50px">
        <div class="wrapper wrapper--w780">
            <div class="card card-3">
                <div class="card-heading dietitian-form">
                    <div class="card-box paragraph-style">
                        <p class="text-center" style="width: 300px"><span class="chef-signup-title-text">Share the
                                Power of Nutrition, Transform Lives</span> <br><br>
                            <span class="chef-signup-paragraph-text">Join Dr. Chef's community of skilled dietitians,
                                helping individuals achieve health goals by sharing free and personalized diet plans.
                                Join now
                                to make a positive impact on lives through nutrition and wellness.</span>
                        </p>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="dregister" enctype="multipart/form-data">
                        @csrf

                        <div class="input-group profile-img-div">
                            <input class="input--style-3" type="file" name="dietitian_profile_pic"
                            ...
                            <br>
                            <label for="profile_pic" style="cursor: pointer;" id="pic_lbl">Upload Image</label>
                        </div>
                        @error('dietitian_profile_pic')
                            <br><label style="color: red; ">{{ $message }}</label>
                        @enderror

                        <br><br><input class="input--style-3 placeholder-text" type="text" placeholder="Name"
                            name="dietitian_full_name"
                            value="{{ old('dietitian_full_name', $data['dietitian_full_name'] ?? '') }}">
                        @error('dietitian_full_name')
                            <br><label style="color: red; ">{{ $message }}</label>
                        @enderror

                        <br><br><input class="input--style-3 placeholder-text" type="text" placeholder="Username"
                            name="dietitian_username"
                            value="{{ old('dietitian_username', $data['dietitian_username'] ?? '') }}">
                        @error('dietitian_username')
                            <br><label style="color: red; ">{{ $message }}</label>
                        @enderror

                        <br><br><input class="input--style-3 placeholder-text" type="email" placeholder="Email"
                            name="dietitian_email"
                            value="{{ old('dietitian_email', $data['dietitian_email'] ?? '') }}">
                        @error('dietitian_email')
                            <br><label style="color: red;">{{ $message }}</label>
                        @enderror

                        <br><br><input class="input--style-3 placeholder-text" type="text"
                            name="dietitian_phone_number" id="tel"
                            value="{{ old('dietitian_phone_number', $data['dietitian_phone_number'] ?? '') }}">
                        <span id="error-msg" class="hide" style="display: inline-block"></span>
                        @error('dietitian_phone_number')
                            <br><label style="color: red; ">{{ $message }}</label>
                        @enderror

                        <br><br><input class="input--style-3 placeholder-text" type="password" placeholder="Password"
                            name="dietitian_password"
                            value="{{ old('dietitian_password', $data['dietitian_password'] ?? '') }}">
                        @error('dietitian_password')
                            <br><label style="color: red; ">{{ $message }}</label>
                        @enderror

                        <br><br>
                        <div class="text-center">
                            <label for="" style="font-weight:bold; font-size:16px">Upload Certificate</label>
                        </div>
                        <input class="input--style-3 placeholder-text" type="file"
                            placeholder="Upload Certificate" name="dietitian_certificate"
                            value="{{ old('dietitian_certificate', $data['dietitian_certificate'] ?? '') }}">
                        @error('dietitian_certificate')
                            <br><label style="color: red; ">{{ $message }}</label>
                        @enderror
                        <div class="p-t-10 text-center">
                            <br><br><small>Already have an account? <a href="{{ route('dietitians/loginform') }}"
                                    class="login-reg-link">Login</a></small>
                            <br><br>
                            <button class="btn btn--pill btn--brown" type="submit">Create Account</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        const iti = window.intlTelInput(input, {
            ...
            preferredCountries: ["pk", "tr", "sa", "us", "gb"]
        });
