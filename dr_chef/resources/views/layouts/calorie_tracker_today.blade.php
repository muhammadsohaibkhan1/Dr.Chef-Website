...
<body>
        ...
        <div class="d-flex flex-column mb-3">
            <div class="p-2">
                <div class="d-flex flex-row mb-3 justify-content-around days-bar">
                    <div class="p-2"><a class="active" href="{{ route('calorietracker/today') }}">Today</a></div>
                    <div class="p-2"><a href="{{ route('calorietracker/weekly') }}">Weekly</a></div>
                    <div class="p-2"><a href="{{ route('calorietracker/monthly') }}">Monthly</a></div>
                </div>
            </div>
            <div class="p-2">
                <div class="d-flex flex-wrap flex-row mb-3 justify-content-around align-items-center"
                    style="height:auto;">
                    <div class="p-2">
                        {{-- Progress bar code block start --}}
                        <div class="circular-progress">
                            <span class="progress-value text-center"></span>
                        </div>
                        {{-- Progress bar code block end --}}
                    </div>
                    <div class="p-2">
                        <b style="font-size: 16px">Extra Intake: </b><span style="float: right; font-size: 16px">
                            @if ($userData->user_daily_calorie_intake > $userData->user_calorie_need)
                                {{ $userData->user_daily_calorie_intake - $userData->user_calorie_need }} Cal.
                            @else
                                0 Cal.
                            @endif
                        </span>
                        ...
                        <br><br><br>
                        <form method="get" action="{{ route('exercises') }}" enctype="multipart/form-data">
                            @csrf
                            <button class="btn btn-outline-secondary btn-block" type="submit">Explore
                                Exercises</button>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Carbohydrates</span><span
                                class="align-right">{{ $userData->user_carbs_intake }} g</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Protein</span><span
                                class="align-right">{{ $userData->user_protien_intake }} g</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Iron</span><span
                                class="align-right">{{ $userData->user_iron_intake }}
                                mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Dietary Fiber</span><span
                                class="align-right">{{ $userData->user_dietaryfiber_intake }} g</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Sugar</span><span
                                class="align-right">{{ $userData->user_sugar_intake }}
                                g</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Calcium</span><span
                                class="align-right">{{ $userData->user_calcium_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Magnesium</span><span
                                class="align-right">{{ $userData->user_magnesium_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Potassium</span><span
                                class="align-right">{{ $userData->user_potassium_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Sodium</span><span
                                class="align-right">{{ $userData->user_sodium_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Vitamin C</span><span
                                class="align-right">{{ $userData->user_vitamin_c_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Vitamin D</span><span
                                class="align-right">{{ $userData->user_vitamin_d_intake }} µg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Vitamin B-6</span><span
                                class="align-right">{{ $userData->user_vitamin_b6_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Vitamin B-12</span><span
                                class="align-right">{{ $userData->user_vitamin_b12_intake }} µg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Cholesterol</span><span
                                class="align-right">{{ $userData->user_cholesterol_intake }} mg</span>
                        </div>
                    </div>
                    <div class="p-2">
                        <div class="nutrient">
                            <span class="align-left">Fats</span><span
                                class="align-right">{{ $userData->user_fats_intake }}
                                g</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="recipe-div">
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th colspan="3">{{ date('l', strtotime($date)) }},
                                        {{ date('F d, Y', strtotime($date)) }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($recipesIntake as $recipes)
                                    <tr>
                                        <td style="width: 100px; white-space: normal;">
                                            <a
                                                href="{{ route('recipe/display/detail', $recipes->recipe_id) }}">{{ $recipes->recipe_name }}</a>
                                        </td>
                                        <td style="width: 40px; white-space: normal;">{{ $recipes->category_name }}
                                        </td>
                                        <td style="width: 40px; white-space: normal;">{{ $recipes->recipe_calories }}
                                            Cal.</td>
                                    </tr>
                                @endforeach
                            </tbody>
    ...
    <script type="text/javascript">
        ...
        let calorieNeed = {{ $userData->user_calorie_need }};
        let calorieIntake = {{ $userData->user_daily_calorie_intake }};
        let calorieIntakePct = Math.floor(calorieIntake / calorieNeed * 100);
        console.log(calorieIntakePct);
        let progressStartValue = 0,
            progressEndValue = calorieIntakePct,
            speed = 20;

        let progress = setInterval(() => {
            progressStartValue++;
            progressValue.textContent = `${calorieIntake}/${calorieNeed} Cal.`;
            circularProgressBar.style.background =
                `conic-gradient(#3D251E, ${progressStartValue * 3.6}deg, #dececa 0deg)`
