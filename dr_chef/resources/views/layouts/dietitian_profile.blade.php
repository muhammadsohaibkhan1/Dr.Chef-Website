...
<head>
    ...
    <!-- Title Page-->
    @foreach ($profile as $dietitian)
        <title>{{ $dietitian->dietitian_full_name }}</title>
    @endforeach
    ...
    <link rel="stylesheet" href="{{ asset('css/dietitian_portal.css') }}" media="all">
    <link rel="stylesheet" href="{{ asset('css/diet_plan_search.css') }}" media="all">
</head>
    ...
    <div class="d-flex flex-wrap main-div">
        <div class="col-12 col-md-3 p-2 d-flex align-items-center justify-content-md-end justify-content-start">
            <div>
                @foreach ($profile as $d)
                    <img src="{{ url('/images/dietitian_profile_images/' . $d->dietitian_profile_pic) }}"
                        alt="profile picture" class="img-fluid profile-image">
                @endforeach
            </div>
        </div>
        <div class="col-12 col-md-9 p-2">
            <div class="profile-info">
                @foreach ($profile as $d)
                    <h4 class="card-text-color mb-1 dietitian-full-name">{{ $d->dietitian_full_name }}</h4>
                    <h6 class="card-text-color mb-3">{{ $d->dietitian_username }}</h6>
                    @if ($accessedBy=='admin')
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                        ...
                        <span> {{ $d->dietitian_likes }} </span>
                    @elseif ($d->is_liked_dietitian == 1)
                        <form action="{{ route('like/dietitian', $d->dietitian_id) }}">
                            <button type="submit" class="like-dietitian-btn">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                    fill="currentColor" class="bi bi-heart-fill" viewBox="0 0 16 16">
                                    <path fill-rule="evenodd"
                                        d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" />
                    ...
                    <br>
                    @if ($accessedBy=='user')
                        <a href='https://api.whatsapp.com/send?phone={{ $d->dietitian_phone_number }}&text=Can I get personalized diet plan"'
                            class="add-diet-plan-btn" target="_blank">Get Personalized Diet Plan</a>
                    @endif
                @endforeach
        ...
        <div class="col-12 p-2">
            <hr>
            <div class="diet-plans-bg">
                <!-- Grid of diet_plans Code Start -->
                <div class="container">
                    <div class="row row-cols-auto" style="row-gap: 25px;">
                        @foreach ($data as $d)
                            <div class="col-md-4">
                                <!-- diet_plan Card Code Start -->
                                        ...
                                        <div class="widget-49">
                                            <div class="widget-49-title-wrapper">
                                                <div class="widget-49-date-primary">
                                                    <span
                                                        class="widget-49-date-day">{{ $d->diet_plan_duration }}</span>
                                                ...
                                                <div class="widget-49-meeting-info">
                                                    <br><span style="font-weight: bold; text-transform:uppercase"
                                                        ...
                                                        {{ $d->diet_plan_type }}
                                                        {{ $d->diet_plan_weight_goal }}kg</span>
                                                </div>
                                            </div>
